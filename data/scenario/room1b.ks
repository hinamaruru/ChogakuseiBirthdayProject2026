[_tb_system_call storage=system/_room1b.ks]

*choice1b

[tb_start_tyrano_code]
[preload storage="./data/bgimage/背景_しゅきめい.png"]
[preload storage="./data/fgimage/default/アイテム背景_改.png"]
[preload storage="./data/fgimage/default/item02.png"]
[_tb_end_tyrano_code]

[cm  ]
[bg  time="0"  method="crossfade"  storage="背景_しゅきめい.png"  ]
[playbgm  volume="70"  time="1000"  loop="true"  storage="グループ１/Halloween_Waltz.mp3"  ]
[mask_off  time="1000"  effect="fadeOut"  ]
[tb_show_message_window  ]
[chara_show  name="超学生"  time="1000"  wait="false"  storage="chara/1/主超　通常.png"  width="800"  height="1000"  left="-180"  ]
[chara_show  name="しゅきしゅきメイドマスカレイド"  time="1000"  wait="true"  storage="chara/15/通常.png"  width="920"  height="1201"  left="228"  top="5"  reflect="false"  ]
[chara_mod  name="しゅきしゅきメイドマスカレイド"  time="0"  cross="false"  storage="chara/15/笑顔.png"  ]
[tb_start_text mode=1 ]
#しゅきしゅきメイドマスカレイド
おかえりなさいませ！ご主人様♡[p]
[_tb_end_text]

[chara_mod  name="超学生"  time="0"  cross="false"  storage="chara/1/主超_焦り_(1).png"  ]
[tb_start_text mode=1 ]
#超学生
あなたは...？[p]
[_tb_end_text]

[chara_mod  name="しゅきしゅきメイドマスカレイド"  time="0"  cross="false"  storage="chara/15/目閉じ_.png"  ]
[tb_start_text mode=1 ]
#しゅきしゅきメイドマスカレイド
見ての通りメイドです♡[p]

[_tb_end_text]

[chara_mod  name="超学生"  time="0"  cross="false"  storage="chara/1/主超_驚き.png"  ]
[tb_start_text mode=1 ]
#超学生
なんで！？[p]
[_tb_end_text]

[chara_mod  name="しゅきしゅきメイドマスカレイド"  time="0"  cross="false"  storage="chara/15/微笑み.png"  ]
[tb_start_text mode=1 ]
#しゅきしゅきメイドマスカレイド
そりゃーメイド喫茶ですからね～[p]
#超学生
メ、メタいっ！！！！[p]

[_tb_end_text]

[chara_mod  name="しゅきしゅきメイドマスカレイド"  time="0"  cross="false"  storage="chara/15/爆笑.png"  ]
[tb_start_text mode=1 ]
#しゅきしゅきメイドマスカレイド
あははっ！超学生様は面白いですねっ！[p]

[_tb_end_text]

[chara_mod  name="超学生"  time="0"  cross="false"  storage="chara/1/主超_焦り_(1).png"  ]
[tb_start_text mode=1 ]
#超学生
...なんで僕の名前を...？[p]

[_tb_end_text]

[chara_mod  name="しゅきしゅきメイドマスカレイド"  time="0"  cross="false"  storage="chara/15/微笑み.png"  ]
[tb_start_text mode=1 ]
#しゅきしゅきメイドマスカレイド
メイドパワーです♡[p]
#超学生
メイドパワー...[p]
[_tb_end_text]

[playse  volume="100"  time="1000"  buf="0"  storage="ドアをドンドン叩く.mp3"  ]
[chara_mod  name="超学生"  time="0"  cross="false"  storage="chara/1/主超_驚き2.png"  ]
[tb_start_text mode=1 ]
#
ドンドン！ドンッ！[p]
#超学生
（......？なんだ、今の......）[p]

[_tb_end_text]

[chara_mod  name="しゅきしゅきメイドマスカレイド"  time="0"  cross="false"  storage="chara/15/ん〜って顔.png"  ]
[mask  time="1000"  effect="fadeIn"  color="0x000000"  ]
[mask_off  time="1000"  effect="fadeOut"  ]
[tb_start_text mode=1 ]
#
数分後[p]
[_tb_end_text]

[chara_mod  name="超学生"  time="0"  cross="false"  storage="chara/1/主超_焦り2.png"  ]
[tb_start_text mode=1 ]
#超学生
（き、気まずい......！）[p]
[_tb_end_text]

[chara_mod  name="しゅきしゅきメイドマスカレイド"  time="0"  cross="false"  storage="chara/15/苦.png"  ]
[tb_start_text mode=1 ]
#しゅきしゅきメイドマスカレイド
今私以外のメイドが居なくて...すみません...💦[p]
[_tb_end_text]

[chara_mod  name="超学生"  time="0"  cross="false"  storage="chara/1/主超　通常.png"  ]
[tb_start_text mode=1 ]
#超学生
全然大丈夫です！お気になさらず！[p]
[_tb_end_text]

[chara_mod  name="しゅきしゅきメイドマスカレイド"  time="0"  cross="false"  storage="chara/15/笑顔.png"  ]
[tb_start_text mode=1 ]
#しゅきしゅきメイドマスカレイド
超学生様は優しいですね[p]
あ！それじゃあせっかくだし、メイドの恋バナ聞いちゃいますか...？笑[p]
#超学生
あ、えっと、ぜひ！[p]
#しゅきしゅきメイドマスカレイド
あはっ！ありがとうございます！[p]
[_tb_end_text]

[chara_mod  name="しゅきしゅきメイドマスカレイド"  time="0"  cross="false"  storage="chara/15/目閉じ_.png"  ]
[tb_start_text mode=1 ]
#しゅきしゅきメイドマスカレイド
それじゃあ、まず彼のことを話しましょうか。[p]
あの人はこの店の常連でした。初回の時私が対応して、それからはずっと私担当でした。[p]
そしてあの人は...ご主人様は...いわゆるリアコ勢でした。[p]
[_tb_end_text]

[chara_mod  name="超学生"  time="0"  cross="false"  storage="chara/1/主超_焦り_(1).png"  ]
[tb_start_text mode=1 ]
#超学生
リアコ......[p]
[_tb_end_text]

[chara_mod  name="しゅきしゅきメイドマスカレイド"  time="0"  cross="false"  storage="chara/15/微笑み.png"  ]
[tb_start_text mode=1 ]
#しゅきしゅきメイドマスカレイド
そうです！最初は面白いなぁぐらいに思ってたんですけど.....たくさん話していくうちに私もご主人様を好きになってしまって...[p]

[_tb_end_text]

[chara_mod  name="しゅきしゅきメイドマスカレイド"  time="0"  cross="false"  storage="chara/15/笑顔.png"  ]
[tb_start_text mode=1 ]
#しゅきしゅきメイドマスカレイド
えへへ、恋しちゃったんですよね...照[p]
[_tb_end_text]

[chara_mod  name="超学生"  time="0"  cross="false"  storage="chara/1/主超_驚き.png"  ]
[tb_start_text mode=1 ]
#超学生
エッ！？！！？禁断の恋ってことッ！？！？！！？[p]
#しゅきしゅきメイドマスカレイド
あはは！そうですね笑　禁断の恋です笑[p]

[_tb_end_text]

[playse  volume="100"  time="1000"  buf="0"  storage="壁をドンと叩く.mp3"  ]
[tb_start_text mode=1 ]
#
ダンッ！！[p]
[_tb_end_text]

[chara_mod  name="超学生"  time="0"  cross="false"  storage="chara/1/主超_驚き.png"  ]
[tb_start_text mode=1 ]
#超学生
！？[p]
[_tb_end_text]

[chara_mod  name="しゅきしゅきメイドマスカレイド"  time="0"  cross="false"  storage="chara/15/ん〜って顔.png"  ]
[tb_start_text mode=1 ]
#しゅきしゅきメイドマスカレイド
......[p]
[_tb_end_text]

[playse  volume="100"  time="1000"  buf="0"  storage="壁をドンと叩く.mp3"  ]
[playse  volume="100"  time="1000"  buf="0"  storage="ドアをドンドン叩く.mp3"  ]
[tb_start_text mode=1 ]

#
ドタッ！ドンドンッ！！！！[p]

[_tb_end_text]

[chara_mod  name="しゅきしゅきメイドマスカレイド"  time="0"  cross="false"  storage="chara/15/微笑み.png"  ]
[tb_start_text mode=1 ]
#しゅきしゅきメイドマスカレイド
すみません、少し様子見てきますね。[p]

[_tb_end_text]

[chara_hide  name="しゅきしゅきメイドマスカレイド"  time="1000"  wait="true"  pos_mode="false"  ]
[playse  volume="100"  time="1000"  buf="0"  storage="ドアをドンドン叩く.mp3"  ]
[tb_start_text mode=1 ]
#
ドタンッ！バタッ！ドンドンッ！！[p]
[_tb_end_text]

[chara_mod  name="超学生"  time="0"  cross="false"  storage="chara/1/主超_焦り_(1).png"  ]
[tb_start_text mode=1 ]
#超学生
......[p]
(なんか、すごい、不穏な...）[p]
[_tb_end_text]

[mask  time="1000"  effect="fadeIn"  color="0x000000"  ]
[mask_off  time="1000"  effect="fadeOut"  ]
[tb_start_text mode=1 ]
#
数分後[p]
[_tb_end_text]

[chara_show  name="しゅきしゅきメイドマスカレイド"  time="1000"  wait="true"  storage="chara/15/通常.png"  width="920"  height="1201"  left="228"  top="5"  reflect="false"  ]
[chara_mod  name="しゅきしゅきメイドマスカレイド"  time="0"  cross="false"  storage="chara/15/微笑み.png"  ]
[tb_start_text mode=1 ]
#しゅきしゅきメイドマスカレイド
すみません💦お待たせしました💦[p]
#超学生
...！？腕から血が......！[p]

[_tb_end_text]

[chara_mod  name="しゅきしゅきメイドマスカレイド"  time="0"  cross="false"  storage="chara/15/仕方ないなーの顔.png"  ]
[tb_start_text mode=1 ]
#しゅきしゅきメイドマスカレイド
あぁ......大丈夫ですよ。私の血じゃないので。[p]
#超学生
え...？じゃあ誰の......[p]

[_tb_end_text]

[chara_mod  name="しゅきしゅきメイドマスカレイド"  time="0"  cross="false"  storage="chara/15/微笑み.png"  ]
[tb_start_text mode=1 ]
#しゅきしゅきメイドマスカレイド
イラナイもののです♡[p]
#超学生
イラナイものって...？[p]
[_tb_end_text]

[chara_mod  name="しゅきしゅきメイドマスカレイド"  time="0"  cross="false"  storage="chara/15/笑顔.png"  ]
[tb_start_text mode=1 ]
#しゅきしゅきメイドマスカレイド
私とご主人様の恋を邪魔する奴らとかぁ他のキャストとか！[p]
邪魔な人たちはサヨウナラしないとですから！[p]
[_tb_end_text]

[chara_mod  name="超学生"  time="0"  cross="false"  storage="chara/1/主超_焦り2.png"  ]
[tb_start_text mode=1 ]
#超学生
...[p]
[_tb_end_text]

[chara_mod  name="しゅきしゅきメイドマスカレイド"  time="0"  cross="false"  storage="chara/15/ウィッグとるとき.png"  ]
[tb_start_text mode=1 ]
#しゅきしゅきメイドマスカレイド
超学生様なら分かってくれますよね？アナタは優しい......そして同じもの同士......[p]
[_tb_end_text]

[chara_mod  name="超学生"  time="0"  cross="false"  storage="chara/1/主超_焦り_(1).png"  ]
[tb_start_text mode=1 ]
#超学生
え..？[p]
[_tb_end_text]

[chara_hide_all  time="0"  wait="true"  ]
[playse  volume="100"  time="1000"  buf="0"  storage="服・マントを脱ぎ着（バサッ）03.mp3"  ]
[tb_start_text mode=1 ]
#
バサッ[p]

[_tb_end_text]

[chara_show  name="超学生"  time="0"  wait="true"  storage="chara/1/主超　通常.png"  width="800"  height="1000"  left="-180"  ]
[chara_mod  name="超学生"  time="0"  cross="false"  storage="chara/1/主超_驚き2.png"  ]
[tb_start_text mode=1 ]
#超学生
......！[p]
[_tb_end_text]

[chara_show  name="しゅきしゅきメイドマスカレイド"  time="1000"  wait="true"  storage="chara/15/ウィッグとったあと.png"  width="920"  height="1201"  left="228"  top="5"  reflect="false"  ]
[chara_mod  name="しゅきしゅきメイドマスカレイド"  time="0"  cross="false"  storage="chara/15/脱　通常.png"  ]
[tb_start_text mode=1 ]
#しゅきしゅきメイドマスカレイド
私はもう一人のアナタです！だから分かってくれますよね？受け入れてくれますよね？だって同一人物なんですから...！同じもの同士なんですから...！[p]
[_tb_end_text]

[chara_mod  name="超学生"  time="0"  cross="false"  storage="chara/1/主超_焦り2.png"  ]
[tb_start_text mode=1 ]
#超学生
なんで、そんなことを......[p]
[_tb_end_text]

[chara_mod  name="しゅきしゅきメイドマスカレイド"  time="0"  cross="false"  storage="chara/15/脱　笑顔.png"  ]
[tb_start_text mode=1 ]
#しゅきしゅきメイドマスカレイド
あははっ！愛する人に愛されたいからです！最初は面白そうだからやってました！[p]

[_tb_end_text]

[chara_mod  name="しゅきしゅきメイドマスカレイド"  time="0"  cross="false"  storage="chara/15/脱　目閉じ.png"  ]
[tb_start_text mode=1 ]
#しゅきしゅきメイドマスカレイド
けどご主人様に恋してからはご主人様のためにずっとこの格好で...！[p]
[_tb_end_text]

[chara_mod  name="しゅきしゅきメイドマスカレイド"  time="0"  cross="false"  storage="chara/15/脱　爆笑.png"  ]
[tb_start_text mode=1 ]
#しゅきしゅきメイドマスカレイド
ご主人様ったら！あんなに大好きって言ってたくせに、本当の私を見たらすごい顔したんですよ！酷い！両想いだったのに！[p]
[_tb_end_text]

[chara_mod  name="しゅきしゅきメイドマスカレイド"  time="0"  cross="false"  storage="chara/15/脱　少し悲しそうな顔.png"  ]
[tb_start_text mode=1 ]
#しゅきしゅきメイドマスカレイド
好きな人のすべてを受け入れるのが愛なのに！私はすべてを受け入れました！[p]
[_tb_end_text]

[chara_mod  name="しゅきしゅきメイドマスカレイド"  time="0"  cross="false"  storage="chara/15/脱　泣き.png"  ]
[tb_start_text mode=1 ]
#しゅきしゅきメイドマスカレイド
惑わされても店に来ない日があっても！ずっとずっと待っていました！！！[p]
[_tb_end_text]

[chara_mod  name="しゅきしゅきメイドマスカレイド"  time="0"  cross="false"  storage="chara/15/脱　怒.png"  ]
[chara_mod  name="超学生"  time="0"  cross="false"  storage="chara/1/主超_驚き2.png"  ]
[tb_start_text mode=1 ]
#しゅきしゅきメイドマスカレイド
なのにご主人様は私のことは受け入れてくれなかった！！！ずっと女装していた時の私しか見てくれなかった！！！！！[p]

[_tb_end_text]

[chara_mod  name="しゅきしゅきメイドマスカレイド"  time="0"  cross="false"  storage="chara/15/脱　圧強め.png"  ]
[tb_start_text mode=1 ]
#しゅきしゅきメイドマスカレイド
目の前にいる本当の私は受け入れてくれなかった！！！！！！！！！！！[p]
[_tb_end_text]

[chara_mod  name="超学生"  time="0"  cross="false"  storage="chara/1/主超_悲しみ.png"  ]
[chara_mod  name="しゅきしゅきメイドマスカレイド"  time="0"  cross="false"  storage="chara/15/脱　絶望泣き.png"  ]
[tb_start_text mode=1 ]
#しゅきしゅきメイドマスカレイド
ただ私は...ご主人様にこっちを見てほしくて...私......私..........！！！！！[p]
[_tb_end_text]

[chara_mod  name="しゅきしゅきメイドマスカレイド"  time="0"  cross="false"  storage="chara/15/ウィッグとったあと.png"  ]
[tb_start_text mode=1 ]
#


[_tb_end_text]

[chara_mod  name="超学生"  time="0"  cross="false"  storage="chara/1/主超_焦り_(1).png"  ]
[chara_mod  name="しゅきしゅきメイドマスカレイド"  time="0"  cross="false"  storage="chara/15/脱　笑顔.png"  ]
[tb_start_text mode=1 ]
#しゅきしゅきメイドマスカレイド
あははっ！それくらいご主人様のことを愛してるんです♡[p]

[_tb_end_text]

[chara_mod  name="しゅきしゅきメイドマスカレイド"  time="0"  cross="false"  storage="chara/15/脱　通常.png"  ]
[tb_start_text mode=1 ]
#しゅきしゅきメイドマスカレイド
...あーそろそろ帰る時間ですね！[p]
[_tb_end_text]

[chara_mod  name="しゅきしゅきメイドマスカレイド"  time="0"  cross="false"  storage="chara/15/脱　表情差分_最後らへん＋おちょに迫るところ.png"  ]
[tb_start_text mode=1 ]
#しゅきしゅきメイドマスカレイド
最後に一緒に踊りましょ...！友達になったしるしに！[p]
[_tb_end_text]

[chara_mod  name="超学生"  time="0"  cross="false"  storage="chara/1/主超_驚き.png"  ]
[tb_start_text mode=1 ]
#超学生
え、あ、わっ[p]
[_tb_end_text]

[tb_start_tyrano_code]
[pausebgm]
[_tb_end_tyrano_code]

[tb_start_tyrano_code]
[playse storage="グループ1/白鳥のワルツ.mp3" buf="1" loop="true"]
[_tb_end_tyrano_code]

[tb_image_show  time="1000"  storage="default/スチル_しゅきめい.png"  width="982"  height="737"  x="-4"  y="-79"  _clickable_img=""  name="img_113"  ]
[tb_start_text mode=1 ]
#
～♪[p]
#しゅきしゅきメイドマスカレイド
あははっ！楽しいですね！超学生様！[p]
#
～♪[p]
[_tb_end_text]

[tb_start_tyrano_code]
[stopse buf="1"]
[changevol target="bgm" volume="0"]
[resumebgm]
[changevol target="bgm" volume="100" time="1500"]
[_tb_end_tyrano_code]

[chara_mod  name="超学生"  time="0"  cross="false"  storage="chara/1/主超_焦り_(1).png"  ]
[chara_mod  name="しゅきしゅきメイドマスカレイド"  time="0"  cross="false"  storage="chara/15/脱　笑顔.png"  ]
[tb_image_hide  time="1000"  ]
[tb_start_text mode=1 ]
#しゅきしゅきメイドマスカレイド
うふふ♡楽しかったですね♡[p]
#超学生
そ、それなら良かった...[p]


[_tb_end_text]

[playse  volume="100"  time="1000"  buf="0"  storage="ドアをドンドン叩く.mp3"  ]
[tb_start_text mode=1 ]
#
ドンッドンッ！！！[p]

[_tb_end_text]

[tb_start_text mode=1 ]
#しゅきしゅきメイドマスカレイド
それではまたいらしてくださいね！超学生様！[p]
[_tb_end_text]

[chara_mod  name="しゅきしゅきメイドマスカレイド"  time="0"  cross="false"  storage="chara/15/脱　目閉じ.png"  ]
[mask  time="1000"  effect="fadeIn"  color="0x000000"  ]
[stopbgm  time="1000"  fadeout="true"  ]
[playbgm  volume="100"  time="1000"  loop="true"  storage="のんびり時間.mp3"  ]
[tb_hide_message_window  ]
[bg  time="0"  method="crossfade"  storage="超学生_防音室_背景.jpg"  ]
[chara_hide  name="しゅきしゅきメイドマスカレイド"  time="0"  wait="false"  pos_mode="true"  ]
[chara_show  name="超学生"  time="0"  wait="true"  left="77"  width="800"  height="1000"  storage="chara/1/主超_真顔.png"  ]
[tb_show_message_window  ]
[chara_mod  name="超学生"  time="0"  cross="false"  storage="chara/1/主超_真顔.png"  ]
[tb_start_text mode=1 ]
#超学生
[_tb_end_text]

[mask_off  time="1000"  effect="fadeOut"  ]
[tb_start_text mode=1 ]
#超学生
ここは......僕の部屋？[p]
さっき見ようとした動画は...なくなっちゃいましたか。[p]
ん？これ...[p]

[_tb_end_text]

[tb_image_show  time="0"  storage="default/アイテム背景_改.png"  width="2400"  height="2400"  name="img_131"  ]
[tb_image_show  time="0"  storage="default/item02.png"  width="292"  height="292"  x="320"  y="171"  _clickable_img=""  ]
[tb_start_text mode=1 ]
#超学生
バラとメッセージカードだ。えーっと...？[p]
#しゅきしゅきメイドマスカレイド
『Dear Friend , Happy Birthday♡』[p]
[_tb_end_text]

[tb_image_hide  time="1000"  ]
[tb_start_text mode=1 ]
#超学生
.........[p]
[_tb_end_text]

[chara_mod  name="超学生"  time="0"  cross="false"  storage="chara/1/主超_目閉じ.png"  ]
[tb_start_text mode=1 ]
#超学生
親愛なる友人、か......[p]
[_tb_end_text]

[chara_mod  name="超学生"  time="0"  cross="false"  storage="chara/1/主超　通常.png"  ]
[tb_start_text mode=1 ]
#超学生
幸せになってくださいねメイドさん...ありがとう。[p]
#
エンド2「しゅきしゅきメイドマスカレイドと僕」[p]
[_tb_end_text]

[tb_eval  exp="sf.item02=1"  name="item02"  cmd="="  op="t"  val="1"  val_2="undefined"  ]
[mask  time="1000"  effect="fadeIn"  color="0x000000"  ]
[bg  time="0"  method="crossfade"  storage="black.png"  ]
[stopbgm  time="1000"  fadeout="true"  ]
[chara_hide  name="超学生"  time="0"  wait="true"  pos_mode="true"  ]
[tb_hide_message_window  ]
[jump  storage="endroll.ks"  target="*endroll"  ]
